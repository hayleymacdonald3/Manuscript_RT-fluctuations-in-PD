function [files] = exp_admin_file_links
    
    private_dir = [fileparts(mfilename('fullpath')) filesep];
    
    files = dir([private_dir '*.m']);
    files = {files.name}';
    files = strtok(files, '.');
...
%% show links to standard files
    ...
    fprintf('Running: ');
    fprintf('%s ', ...
        link_text('edit', [private_dir 'exp_run.m'], 'run'), ...
        link_text('edit', [private_dir 'exp_trial.m'], 'trial'), ...
        link_text('edit', [private_dir 'exp_trial_get_response.m'], 'get'), ...
        link_text('edit', [private_dir 'exp_trial_check_response.m'], 'check'), ...
        link_text('edit', [private_dir 'exp_trial_feedback.m'], 'feedback'));
    fprintf('\n');
    fprintf('Initializing: ');
    fprintf('%s ', ...
        link_text('edit', [private_dir 'exp_initialize.m'], 'initialize'), ...
        link_text('edit', [private_dir 'exp_data_generate.m'], 'trial_data'), ...
        link_text('edit', [private_dir 'exp_initialize_edata.m'], 'edata'), ...
        link_text('edit', [private_dir 'exp_initialize_audio.m'], 'audio'), ...
        link_text('edit', [private_dir 'exp_initialize_display.m'], 'display'), ...
        link_text('edit', [private_dir 'exp_initialize_inputs.m'], 'inputs'));
    fprintf('\n');
...
%% show links to custom files
    ...
    for group_num = 1:length(custom_groups)
        ...
        for x = 1:size(group_files,1)
            ...
            fprintf('%s ', link_text('edit', [private_dir char(group_files.name(x))], short_name))
        end
        fprintf('\n');
    end
